...
async function touchEvent({ target }: { target: Composite }) {
    ...
    if (fs.isDir(path)) {
        if (isReader) {
            const wrapParentFolder: WidgetCollection = target
                .parent()
                .siblings();
...
function ComponentFileOption({
    ...
    const icon = getIconPath(
        name,
        type === TypeFile.DIRECTORY ? TypeIcon.DIRECTORY : TypeIcon.FILE
    );
    const data: DataView = {
        path,
        name,
    ...
    const wrap = (
        <Composite data={data} padding={{ left }} top="prev() 2">
            <Row alignment="bottom">
                <Composite
                    ...
                    padding={[5, 30, 5, 5]}
                >
                    <ImageView scaleMode="stretch" image={icon} {...size} />
                    <TextView
                        text={name}
                        class="text-filename"
                        textColor="white"
...
async function resolveDialogCreate(
    dialog: AlertDialog
...
function insertViewTo(
    path: string,
    textComponentFile: string,
    type: TypeFile,
    excludeLayout: boolean,
    keyStoreFile: TypeKey
) {
    ...
    const container = (
        isRoot ? drawer.find("#viewFiles").only() : widgetStore
    ) as Composite;
    // obtener los hijos del widget padre
    const collection = isRoot
        ? container.children()
        : container.children().slice(1);
    ...
    const findIndex = filterCollection
        .toArray()
        .findIndex((child: Composite, index: number) => {
...
async function touchMenuOption(view: Composite): Promise<any> {
    const { name, path, type, isReader, isOpen }: DataView = view.data;
            ...
            actionsheet.items.push(
                {
                    title: "Crear Archivo",
                    async action() {
                        const name = await resolveDialogCreate(
                            DialogTextInput({
                                title: "nuevo archivo",
                                message: "nombre.txt",
                                btnOk: "crear",
    ...
    const actionSheet = ActionSheet.open(
        <ActionSheet title={(actionsheet.title as any).toCapitalize()} actions={items}>
            {actionsheet.message}
        </ActionSheet>
    );
