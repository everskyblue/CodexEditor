import { fileIcons } from "./material-icon-theme/fileIcons";
import { folderIcons } from "./material-icon-theme/folderIcons";
...
export const getNameIcon = (
    nameFile: string,
    resourceType: TypeIcon
): string => {
    if (resourceType === TypeIcon.DIRECTORY) {
        const res = folderIcons[0].icons.find((def) =>
            def.folderNames.includes(nameFile)
        );
    ...
    const res = fileIcons.icons.filter((def) => {
        if ("fileNames" in def && def.fileNames.includes(nameFile)) {
            ...
            return def.fileExtensions.some((ext) => {
                ...
                return (
                    nameFile.substring(nameFile.lastIndexOf(ext) - 1) ===
                    `.${ext}`
                );
            });
        }
...
export const getIconPath = (nameFile: string, typeIcon: TypeIcon) =>
    join(SRC_IMAGE, getNameIcon(nameFile, typeIcon).concat(".png"));

export {default as ImageBlob} from "./base64";
