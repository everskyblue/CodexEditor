import { ActionSheet, ActionSheetItem, drawer, fs } from "tabris";
import { getProjectSettings, saveProject, removeProject } from "../store";
...
async function showProjects() {
    ...
    const action = ActionSheet.open(
        <ActionSheet
            title="Lista de Proyectos Abiertos"
            message={projects.length === 0 ? "no hay proyectos" : ""}
        >
            {projects.map((path) => {
                return (
                    <ActionSheetItem
